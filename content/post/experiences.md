---
title: "Experiences"
date: 2025-11-15T15:28:12+02:00
draft: false
tags: ["Expériences"]
---
Mes expériences professionnelles
<!--more-->
### Agent d'accueil / d'entretien - Aquabaie, Avranches
- **Date**: Octobre 2024 - Actuellement
- **Description**:
  - Réception et assistance des clients, garantissant un service de qualité et une satisfaction optimale.
  - Entretien rigoureux des locaux pour maintenir un environnement impeccable et accueillant.
  - Gestion de la caisse, incluant la supervision des transactions financières, la manipulation des espèces et la tenue précise des registres comptables.

### Stagiaire Développeur Web - La Prohibition, Jullouville
- **Date**: De mai à juin 2025
- **Description**:
  - Création d'un site web vitrine de A à Z pour le bar restaurant La Prohibition, mettant en avant ses services et événements.
  - Utilisation du framework Next.js pour le développement front-end, assurant une interface utilisateur moderne et réactive.
  - Intégration d'une base de données Supabase pour la gestion dynamique du contenu du site.

### Stagiaire Informatique - Happy Cash, Vire
- **Date**: Stage de 4 semaines en 2023
- **Description**:
  - Accueil et conseil de la clientèle, fournissant des recommandations adaptées aux besoins spécifiques de chaque client.
  - Vente de matériel informatique varié, avec une expertise technique permettant d'orienter efficacement les clients.
  - Réparation d'ordinateurs portables et fixes, ainsi que de consoles de jeu, en effectuant des diagnostics précis et des interventions techniques.
  - Gestion des appels téléphoniques avec la clientèle, assurant un support à distance et un suivi après-vente de qualité.

### Stagiaire Informatique - Phone Expert, Coutances
- **Date**: Stage de 4 semaines en 2022
- **Description**:
  - Accueil et orientation de la clientèle pour l'achat de téléphones et d'accessoires, en mettant en avant les caractéristiques techniques et les avantages des produits.
  - Vente de téléphones et d'accessoires, en fournissant des conseils personnalisés pour répondre aux attentes des clients.
  - Réparation d'ordinateurs portables et fixes, de consoles de jeu et de téléphones, en appliquant des compétences techniques pour résoudre divers problèmes matériels et logiciels.

### Stagiaire Informatique - IoBuro, Coutances
- **Date**: Plusieurs stages entre 2021 et 2022
- **Description**:
  - Accueil et accompagnement de la clientèle dans le choix de matériel informatique adapté à leurs besoins professionnels et personnels.
  - Vente de matériel informatique divers, en fournissant des explications détaillées sur les produits et leurs utilisations.
  - Réparation d'ordinateurs portables et fixes, en effectuant des diagnostics précis et des réparations efficaces.
  - Impression et mise en service d'imprimantes professionnelles, en assurant leur bon fonctionnement et en formant les utilisateurs à leur utilisation.
  - Maintenance de postes informatiques, en veillant à la mise à jour des systèmes et à la résolution des problèmes techniques pour garantir une utilisation optimale.x
